...
def merge_configs(base_cfg: CommentedMap, cfg: Union[CommentedMap, dict]) -> CommentedMap:
...
def fit_config(**kwargs) -> CommentedMap:
    ...
    with open("./simple_classifier/configs/base.yml") as f:
        base_config = yaml.load(f)

    if "config" in kwargs:
        cfg_name = kwargs.pop("config")
        with open(f"./simple_classifier/configs/{cfg_name}", "r") as f:
            yaml_config = yaml.load(f)
...
def object_from_dict(d: Dict, *args, **default_kwargs) -> Any:
    """
    Loads python object from cfg dict with params.
    Recursively executed if the parameters also need to be loaded from the dictionary.
    """
    kwargs = deepcopy(dict(d))
    ...
    # recursively load objects from params dict
    for name in params:
        value = params[name]
        if isinstance(value, dict) and "type" in value.keys():
            params[name] = object_from_dict(d["params"][name])

    try:
        if params is not None:
            return pydoc.locate(object_type)(*args, **params)  # type: ignore
        else:
